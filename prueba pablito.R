...
X <- data$alto
alpha <- 0.01
t.test(X, alternative = 'less' ,mu = 3.6 ,conf.level = 1-alpha)
...
X <- filter(data,centro=="Chiloé")$alto

Y <- filter(data,centro=="Puerto Montt")$alto

alpha2 <- 0.05

var.test(X, Y, alternative="two.sided", conf.level = 0.95, ratio=1)
